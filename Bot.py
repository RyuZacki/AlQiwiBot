...
def keyboard(where_call):
    if where_call == 'Categories':
        ...
        markup = types.InlineKeyboardMarkup(row_width=1)
        MinimumAccount = types.InlineKeyboardButton("Минимальный аккаунт | 300 ₽ | Кол-во: 44 шт.",
                                                    callback_data='MinimumAccount')
        AverageAccount = types.InlineKeyboardButton("Средний аккаунт | 1000 ₽ | Кол-во: 21 шт.",
                                                    callback_data='AverageAccount')
        ProfessionalAccount = types.InlineKeyboardButton("Профессиональный аккаунт | 2400 ₽ | Кол-во: 14 шт.",
                                                         callback_data='ProfessionalAccount')
...
def welcome(message):
    ...
    bot.send_message(message.chat.id, "Здравствуйте✌\n"
                                      ...
                                      "И воспользуйтесь меню👇", parse_mode='html', disable_web_page_preview=True,
                     reply_markup=markup)
...
def menu(message):
    if message.chat.type == 'private':
        if message.text == 'Купить Qiwi аккаунт🥝':
            bot.send_message(message.chat.id, "Активные категории в магазине:", parse_mode='html',
            ...
            bot.send_message(message.chat.id, '🔐 ID: {id}\n'
                                              ...
                                              '💸 Количество покупок: 0'.format(id=IDUser, nameuser=UserName),
                             parse_mode='html', reply_markup=keyboard('Profile'))
            ...
            bot.send_message(message.chat.id, '🏠 Магазин: 🥝WalletsQiwiPocket💳\n'
                                              ...
                                              '📢 Канал:  {Text}'.format(Text=text1), disable_web_page_preview=True,
                             parse_mode='Markdown')
    ...
    try:
        if call.message:
            if call.data == 'Balance':
                ...
                bot.delete_message(call.message.chat.id, call.message.message_id)
                bot.send_message(call.message.chat.id, "Активные категории в магазине:",
                                 reply_markup=keyboard('Categories'))
                ...
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                      text="Заказ отменен", reply_markup=None)
